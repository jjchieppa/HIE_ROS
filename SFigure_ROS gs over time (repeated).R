...
tiff(file = "Visual_ROS gs over time (repeated).tiff", height = 8, width = 12, res = 600,units = "in", compression = "zip+p")

par(mfrow = c(3,1), mar = c(0, 0, 0, 0), cex.lab = 1.3, omi = c(1, 1, 0.5, 1))

plot(gs.max ~ Date, xrye, pch = "", ylim = c(0,850), xaxt = "n", yaxt = "n")
axis(2, at = seq(0,750,150), cex.axis = 1.5, las = 2)
points(gs.max ~ Date, crye, pch = 1, cex = 2, col = "black")
points(gs.max ~ Date, xrye, pch = 1, cex = 2, col = "red")
abline(v = as.Date("2018-6-20"), lty = 3)
abline(v = as.Date("2018-9-6"), lty = 3)
abline(v = as.Date("2018-10-25"), lty = 3)
abline(v = as.Date("2018-11-12"), lty = 3)
abline(v = as.Date("2018-11-22"), lty = 3)
abline(h = 25, lty = 2, col = "grey")
legend("top", expression(italic("L. perenne")), bty = "n", cex = 1.5)

par(new = T)
points(as.numeric(ROS3$ROS3_A_P1_Avg.mean*5000) ~ ROS3$Date, type = "l", lty = 1) #Amb
points(as.numeric(ROS3$ROS3_X_F3_Avg.mean*5000) ~ ROS3$Date, type = "l", lty = 1, col = "red") #short
axis(4, at = seq(0,750,150), labels = c("0","3","6","9","12","15"), cex.axis = 1.5, las = 2)

plot(gs.max ~ Date, xrye, pch = "", ylim = c(0,850), xaxt = "n", yaxt = "n")
axis(2, at = seq(0,750,150), cex.axis = 1.5, las = 2)
points(gs.max ~ Date, cfes, pch = 1, cex = 2, col = "black")
points(gs.max ~ Date, xfes, pch = 1, cex = 2, col = "red")
abline(v = as.Date("2018-6-20"), lty = 3)
abline(v = as.Date("2018-9-6"), lty = 3)
abline(v = as.Date("2018-10-25"), lty = 3)
abline(v = as.Date("2018-11-12"), lty = 3)
...
points(gs.max ~ Date, xpha, pch = 1, cex = 2, col = "red")
abline(v = as.Date("2018-6-20"), lty = 3)
abline(v = as.Date("2018-9-6"), lty = 3)
abline(v = as.Date("2018-10-25"), lty = 3)
abline(v = as.Date("2018-11-12"), lty = 3)
...
axis.Date(1, lrye$Date, at = seq(as.Date("2018-4-20"), as.Date(max(xrye$Date)), "weeks"),
...
mtext(side = 3, outer = T, "Repeated Drought", cex = 1.5, padj = 0)
